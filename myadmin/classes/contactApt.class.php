...
class contactApt extends Dbh {
    public function contact_apt_form($post_token,$name,$mobile,$email,$subject,$msg){
        $csrf_token = $_SESSION['csrf_token'];
        $post_token = $_POST['csrf_token'];

        if(hash_equals($csrf_token,$post_token))
        {
            if( (!empty($_POST['s_name']) && !empty( $_POST['s_email']) && !empty($_POST['s_mobile']) && !empty($_POST['s_desc']) ) )
            {  
                ...
                try {
                    ...
                    $mail->Subject = "Message Received from $name";
                    $mail->Body    = "
...
<p style='font-size: 14px; line-height: 140%;'><span style='font-size: 18px; line-height: 25.2px; color: #666666;'>Apartment Name:    $apt_name</span></p>
...
<p style='font-size: 14px; line-height: 140%;'>&nbsp;</p>
<p style='font-size: 14px; line-height: 140%;'><span style='font-size: 18px; line-height: 25.2px; color: #666666;'>Message:     $description</span></p>
<p style='font-size: 14px; line-height: 140%;'>&nbsp;</p>
  </div>
...
";
                    $mail->AltBody = "Apartment Name : $apt_name\nName : $name\nEmail : $email\nMobile No. : $mobile\nMessage Description : $description\n";
                    $result=$mail->send();
                    if($result)
                    if($result)
                    {
